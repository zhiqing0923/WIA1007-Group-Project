...
server <- function(input, output, session) {
  ...
  output$universityAndMigrationRate<- renderPlot({
    if(input$purposeOfVisit == "Study Abroad"){
      ...
      ggplot(migrate %>% filter(ï..Country == input$selectCountries),aes(x=year,y=migrants,group = 1)) + 
  ...
  output$image<-renderUI({
    if(input$purposeOfVisit == "Holiday"){
      ...
      else if(input$selectCountries == "Spain")img(src = "spain_travel.png", h1("Sagrada Familia"), h3("The Basilica de la Sagrada Familia also known as the Sagrada Familia, is a large unfinished minor basilica in the Eixample district of Barcelona, Catalonia, Spain. Designed by the Catalan architect Antoni Gaudi (1852-1926), his work on the building is part of a UNESCO World Heritage Site. On 7 November 2010, Pope Benedict XVI consecrated the church and proclaimed it a minor basilica."), h5(tags$a(href="https://en.wikipedia.org/wiki/Sagrada_Fam%C3%ADlia","Click here for more information"))) ##Sagrada FamÃ­lia
